...
void handle_signal(int sig) {
    ...
    char cwd[PATH_MAX];
    if (getcwd(cwd, sizeof(cwd)) != NULL && getlogin() != NULL) {
        char* user = getlogin();
        printf("\e[1;31m%s$\e[1;32m%s\e[0m", user, cwd);
    }
    fflush(stdout);
}
...
void setup_signal_handlers() {
    ...
    term.c_cc[VEOF] = _POSIX_VDISABLE;  // Отключаем символ EOF
...
int execute_pipe(node* root){
    ...
    if (cpid == 0){ // child
        ...
        close(pipefd[0]);
        int st = execute_tree(root->right);
